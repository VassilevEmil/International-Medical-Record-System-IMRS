...
router.post('/medicalRecord', async (req: Request, res: Response) => {
  try {
    const { doctorFirstName, doctorLastName, sendingInstitution, diagnosisName, contentText, contentImage, language, patientId } = req.body;
    
    const medicalRecord = await createMedicalRecord(patientId, doctorFirstName, doctorLastName, sendingInstitution, diagnosisName, contentText, contentImage, language);
