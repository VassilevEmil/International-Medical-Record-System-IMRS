...
export default class GetDrugsService {
  private static BASE_URL =
    "https://imrs-server-12m3e12kdk1k12mek.tech/api/drugRecords/getAllDrugRecords/";

  static async fetchDrugRecordsByPatientId(
    patientId: string,
    page: number,
    recordLimit: number
  ): Promise<GetDrugResponse> {
    const queryParams = new URLSearchParams({
      page: page.toString(),
      recordLimit: recordLimit.toString(),
    });

    const urlWithPatientIdAndParams = `${this.BASE_URL}${encodeURIComponent(
      patientId
